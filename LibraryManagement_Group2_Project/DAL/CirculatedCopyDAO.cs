...
namespace LibraryManagement_Group2_Project.DAL
{
    class CirculatedCopyDAO
    {
        ...
        public static DataTable GetMonthlyBorrowingTrends()
        {
            string sql = "SELECT MONTH(borrowedDate) AS Month, COUNT(*) AS TotalBorrowed " +
                         "FROM CirculatedCopy " +
                         "WHERE borrowedDate IS NOT NULL " +
                         "GROUP BY MONTH(borrowedDate) " +
                         "ORDER BY MONTH(borrowedDate)";
