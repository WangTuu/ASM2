...
namespace LibraryManagement_Group2_Project.DAL
{
    class MemberDAO
    {
        ...
        public static DataTable GetCbos()
        {
            string sql = "select memberNumber, name from Member";
            return DAO.GetDataTable(sql);
        }
        ...
        public static DataTable GetTotalBooksBorrowed(int memberNumber)
        {
            string sql = "SELECT COUNT(*) AS TotalBorrowed " +
                         "FROM CirculatedCopy WHERE memberNumber = @memNum AND returnedDate IS NULL";
        ...
        public static int GetLateReturnsCount(int memberNumber, DateTime startDate, DateTime endDate)
        {
            string sql = "SELECT COUNT(*) AS TotalLateReturns " +
                         "FROM CirculatedCopy " +
                         "WHERE memberNumber = @memNum AND returnedDate IS NOT NULL AND returnedDate > dueDate " +
                         "AND borrowedDate BETWEEN @startDate AND @endDate";
            SqlCommand cmd = new SqlCommand(sql);
            cmd.Parameters.AddWithValue("@memNum", memberNumber);
            ...
            {
                return Convert.ToInt32(dt.Rows[0]["TotalLateReturns"]);
            }
            return 0;
        }
